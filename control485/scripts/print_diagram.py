...
if __name__ == '__main__':
    car_speed = np.load("m87_car_speed.npy", allow_pickle=True)
    reel_speed = np.load("m87_reel_speed.npy", allow_pickle=True)
    cb_speed = np.load("m87_cb_speed.npy", allow_pickle=True)
    reel_current = np.load("m87_reel_current.npy", allow_pickle=True)
    cb_current = np.load("m87_cb_current.npy", allow_pickle=True)
    cm7290_current = np.load("m87_cm7290_current.npy", allow_pickle=True)
    ...
    ax5.step(car_speed[..., 0], car_speed[..., 1], 'o', where='post', label='car_speed')
    # 设置坐标刻度大小
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    # 设置坐标标签字体大小
