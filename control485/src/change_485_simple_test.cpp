...
//    sub_motor_current_cm7290 = n_.subscribe("current_cm7290", 10, &current_cm7290_cb);
    ...
    n_.getParam("motor_id", motor_id);
    n_.getParam("target_speed", targetSpeed);
...
//    filename = filename + current_time + "m10_1500_speed.txt";
    ...
    motorSetSpeed(motor_id, targetSpeed);

    double sec_count = 0.0, sec_duration = 200.0;
    while (ros::ok() && sec_count <= sec_duration)
    {
        // Read and save motor speed;
//        if (int(sec_count) % 2 == 0 && targetSpeed <= 2000){
//            targetSpeed += 150;
//            motorSetSpeed(motor_id, targetSpeed);
//            usleep(50000);
//        }
        ...
        cout<<"Real speed is "<<realSpeed<<endl;
//        usleep(5000);
        sec_count += 0.005;
    }
//    sleep(1);
...
void *test_speed_control(void *) {
    ...
    motorSetSpeedmode(1);
    int targetSpeed = 0;
    while (targetSpeed <= 2500){
        motorSetSpeed(1, targetSpeed);
        sleep(2);
        targetSpeed += 100;
    }
